...
Element::Element(int _key, std::string _value, Element* _parent, Element* _right, Element* _left)
{
    key = _key;
    right = _right;
    left = _left;
...
Element* BinaryTree::CopyTree(Element* tmp, Element* parent)
{
    if(tmp == 0)
        return 0;
    Element *NewElement = new Element(tmp->getKey(), tmp->getValue(), tmp->getParent() );
...
BinaryTree& BinaryTree::operator=(BinaryTree& tmp)//перегрузка
{
...
Element* BinaryTree::searchByKeyInSubtree(const int& key, Element* start)
{
    ...
    else return searchByKeyInSubtree(key, start->getRight());
}
Element* BinaryTree::searchByValueInSubtree(const std::string& value, Element* start)
{
    ...
    else return searchByValueInSubtree(value, start->getRight());
}
