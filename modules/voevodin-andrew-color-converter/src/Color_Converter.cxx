...
vector<int> color_converter::XYZtoRGB(const vector<double> _XYZ) const{
    ...
    double var_G = var_X * (-0.9693660) + var_Y *  1.87560108 + var_Z *  0.0415560;
...
vector<double> color_converter::XYZtoHSV(const vector<double> _XYZ) const{
    ...
    else if (RGB[1] >= RGB[2]) HSV[0] = 60.0*(RGB[1] - RGB[2]) / (max - min);
    else if (RGB[1] < RGB[2]) HSV[0] = 60.0*(RGB[1] - RGB[2]) / (max - min) + 360.0;
    else if (RGB[1] == max) HSV[0] = 60.0*(RGB[2] - RGB[0]) / (max - min) + 120.0;
